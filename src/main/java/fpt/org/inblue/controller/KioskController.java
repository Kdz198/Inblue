...
public class KioskController {
    ...
    @Operation(summary = "Lấy danh sách các Kiosk đang hoạt động", description = "Trả về danh sách tất cả các trạm Kiosk vật lý có trạng thái hoạt động (isActive = true).")
    @GetMapping
    public ResponseEntity<List<Kiosk>> getActiveKiosks() {
        return ResponseEntity.ok(kioskService.getActiveKiosks());
    }

    @Operation(summary = "Lấy danh sách các slot trống của Kiosk", description = "Tính toán và trả về danh sách các khung giờ (slot) còn trống của trạm Kiosk được chọn trong một ngày cụ thể (bao gồm khoảng nghỉ 15 phút giữa các slot).")
    ...
    @Operation(summary = "Tạo một Kiosk mới", description = "Tạo cấu hình cho một máy trạm Kiosk vật lý mới trong hệ thống (chỉ dành cho Admin).")
    @PostMapping
    public ResponseEntity<Kiosk> createKiosk(@RequestBody Kiosk kiosk) {
        return ResponseEntity.ok(kioskService.createKiosk(kiosk));
    }

    @Operation(summary = "Thiết lập lịch hoạt động định kỳ của Kiosk", description = "Tạo cấu hình lịch làm việc (giờ mở cửa, đóng cửa, độ dài slot) định kỳ theo thứ trong tuần của một Kiosk (chỉ dành cho Admin).")
    ...
    @Operation(summary = "Lấy danh sách lịch hoạt động định kỳ của Kiosk", description = "Trả về danh sách các cấu hình lịch làm việc theo thứ của một Kiosk.")
    ...
    @Operation(summary = "Cập nhật thông tin Kiosk", description = "Chỉnh sửa tên, vị trí hoặc trạng thái hoạt động của một Kiosk vật lý hiện có (chỉ dành cho Admin).")
    @PutMapping("/{id}")
    ...
    @Operation(summary = "Cập nhật lịch hoạt động định kỳ của Kiosk", description = "Chỉnh sửa thông số lịch làm việc định kỳ (giờ mở, giờ đóng, độ dài slot) của Kiosk theo mã lịch (chỉ dành cho Admin).")
