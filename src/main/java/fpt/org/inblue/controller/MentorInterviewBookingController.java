...
public class MentorInterviewBookingController {
    ...
    @Operation(summary = "Ứng viên đặt lịch phỏng vấn tại Kiosk", description = "Cho phép ứng viên chọn một trạm Kiosk và slot thời gian còn trống để đặt lịch phỏng vấn (tạo booking trạng thái AWAITING_MENTOR).")
    ...
    @Operation(summary = "Hủy/đổi lịch phỏng vấn", description = "Hủy lịch phỏng vấn hiện tại. Chuyển trạng thái booking thành CANCELLED và reset trạng thái vòng thi ứng tuyển về PENDING để ứng viên có thể chọn lại slot mới.")
    ...
    @Operation(summary = "Lấy danh sách các lượt phỏng vấn theo trạng thái", description = "Lấy toàn bộ các lượt đặt lịch phỏng vấn dựa theo trạng thái lọc (chỉ dành cho Admin/Staff). Mặc định là AWAITING_MENTOR.")
    ...
    @Operation(summary = "Gán Mentor cho lịch phỏng vấn", description = "Admin thực hiện gán Mentor cho một lượt phỏng vấn. Hệ thống kiểm tra trùng lịch Mentor, gọi Daily.co để tạo phòng, sinh mã sessionKey và gửi thông báo cho ứng viên (chỉ dành cho Admin).")
    ...
    public ResponseEntity<MentorInterviewBooking> assignMentor(
            ...
            @RequestBody AssignMentorDtoRequest dto) {
        return ResponseEntity.ok(bookingService.assignMentor(bookingId, dto.getMentorId(), dto.getNotes()));
    }

    @Operation(summary = "Kiosk xác thực và vào phòng phỏng vấn", description = "Máy Kiosk vật lý gửi sessionKey và kioskId để xác thực. Hệ thống kiểm tra thời gian hợp lệ (±15 phút so với giờ hẹn) và sinh Daily.co meetingToken cho ứng viên.")
