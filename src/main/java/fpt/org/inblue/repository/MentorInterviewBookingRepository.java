...
public interface MentorInterviewBookingRepository extends JpaRepository<MentorInterviewBooking, Long> {
    ...
    @Query(
            "SELECT b FROM MentorInterviewBooking b WHERE b.kioskId = :kioskId AND b.status <> :cancelledStatus AND b.scheduledStart >= :start AND b.scheduledEnd <= :end")
    ...
    @Query(
            "SELECT COUNT(b) FROM MentorInterviewBooking b WHERE b.kioskId = :kioskId AND b.status <> :cancelledStatus AND b.scheduledStart < :end AND b.scheduledEnd > :start")
    ...
    @Query(
            "SELECT COUNT(b) FROM MentorInterviewBooking b WHERE b.mentorId = :mentorId AND b.status <> :cancelledStatus AND b.scheduledStart < :end AND b.scheduledEnd > :start")
